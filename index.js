...
function profilListesi(fenomen) {
  let kopya =[...fenomen]
  for (let index = 0; index < kopya.length; index++) {
    kopya[index] = kopya[index].profile
...
function fenomenSil(fenomenler,indeks) {
  let kopya =[...fenomenler]
  kopya.splice(indeks,1)
  return kopya
}
...
function enFenomenler(fenomenler) {
  ...
  for (let index = 0; index < fenomenler.length; index++) {
    if (fenomenler[index].followers > 100000000 ) {
      newArray.push(fenomenler[index].profile)
    }
  }
  return newArray
}
...
function platformaGoreCokGonderiYapanFenomen(fenomenler,platformAdi){
  let gonderiSayisi = 0
  let enCokGonderisiOlanFenomen = ""
  for (let index = 0; index < fenomenler.length; index++) {
    const element = fenomenler[index];
    if (element.platform == platformAdi) {
     if (gonderiSayisi < element.posts ) {
      gonderiSayisi = element.posts
      enCokGonderisiOlanFenomen = element.profile
     } 
    }
  }
  return enCokGonderisiOlanFenomen
}
